...
public class Golf extends CardGame {
    ...
    protected static HashMap<CardFace, Integer> specialCards = new HashMap<CardFace, Integer>();
    ...
    public Golf(String override){
        super(override);
        specialCards.put(CardFace.KING, 0);
        specialCards.put(CardFace.TWO, -2);
        specialCards.put(CardFace.ACE, 1);
    }
    ...
    protected Card playerSelectCard(Player player, Deck deck){
        displayCard("Discard Pile", discardPile.getLastCard());
    ...
    protected int scoreHand(Hand hand){
        int score = 0;
        if (hand.getCard(0).getCardFace() != hand.getCard(1).getCardFace()){
            for (Card card : hand.getHandOfCards()){
                if (specialCards.containsKey(card.getCardFace())){
                    score += specialCards.get(card.getCardFace());
                }
                else {
    ...
    protected boolean compareCardWithHand(Hand hand, Card cardPlayed){
        ...
        while (!matchedCard && counter < hand.size()){
            ...
            if (cardPlayed.getCardFace() == computerCard.getCardFace()){
                matchedCard = true;
                if (counter == 0){
                    counter = 1;
                } else {
